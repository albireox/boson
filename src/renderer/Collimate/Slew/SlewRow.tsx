...
export default function SlewRow() {
  ...
  const mirrorSelect = (event: SelectChangeEvent) => {
  ...
  async function exposeTwo() {
    ...
    await window.electron.tron.send(
      `tcc offset inst 0, -1.47 /pabsolute/computed`
    );
  ...
  async function rotateMirror() {
    ...
    const xtrans: number =
      Math.round(mirTrans * Math.cos((gfaAng * Math.PI) / 180.0) * 100.0) /
      100.0;
    const ytrans: number =
      Math.round(mirTrans * Math.sin((gfaAng * Math.PI) / 180.0) * 100.0) /
      100.0;
    const xtilt: number =
      Math.round(-1.0 * ytrans * tiltFactor * 100.0) / 100.0; // x tilt opposite sign as ytrans
